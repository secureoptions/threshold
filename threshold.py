...
# If host ip exists, assume user is setting up a monitor
    ...
    if len(hostip_list) > 1:
        ...
        def http_monitor(fork_pid):
            ...
            if IPV6:
                args = 'curl -g -6 -o /dev/null -s -k -m {} "{}"' .format(TIMEOUT,HOSTIP)
            else:
                args = 'curl -o /dev/null -s -k -m {} {}' .format(TIMEOUT,HOSTIP)
            ...
            while True:
                # Check that HTTP(s) response code is 2xx or 3xx, but fail if anything else
                p = subprocess.Popen('curl -I -s -k ' + HOSTIP, shell=True, stdout=subprocess.PIPE).communicate()[0].decode('utf-8')

                m = re.search(r'HTTP/1.1 (\d+)', p)
                if m:
                    code = m.group(1)
                    if int(code[0]) not in [2,3]:
                        # Get defined action
                        ...
                        logging.warning(str(fork_pid) + ' Monitor failed due HTTP(s) code (' + code + '): ' + args)
                ...
                if p.returncode == 0:
                    ...
                    # Log action
                    if p.returncode == 28:
                        logging.warning(str(fork_pid) + ' Monitor failed due to taking too long: ' + args)
                    else: 
                        logging.warning(str(fork_pid) + ' failed with curl error code \'' + p.returncode + '\'')
                        logging.warning(str(fork_pid) + ' reference error codes at https://curl.haxx.se/libcurl/c/libcurl-errors.html')
                    logging.warning('Took action for ' + str(fork_pid) + ': ' + action)
